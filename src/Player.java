...
public class Player {
	...
	private Color color;
	private String id;
	private double v_limitUpper;
	private double v_limitLower;
	...
	*/
	public Player(double cx, double cy, double width, double height, Color color, String id, double [] v_limit, double speed){
		...
		this.color = color;
		this.id = id;
		this.v_limitUpper = v_limit[0];
		this.v_limitLower = v_limit[1];
		this.speed = speed;
	}

	/**
		Método chamado sempre que o player precisa ser (re)desenhado.
	 	Desenha ambos os jogadores com apenas "uma" chamada. Atribui-se a cor do objeto que
	 	chama o método, e preenche o retângulo de acordo com o argumento de entrada.
	*/
	public void draw(){
	...
	/**
		...
		de mover o player para cima estiver pressionada.
	 	O centro da coordenada do retângulo não deve ser menor que
	 	o limite da borda (v_limitUpper) mais a metade da sua altura
	 	(considerando que a coordenada Y esteja no centro do retângulo).

	 	O movimento deste retângulo só ocorre se essa condição for respeitada.

		@param delta quantidade de millisegundos que se passou entre o ciclo anterior de atualização do jogo e o atual.
	*/
	public void moveUp(long delta){
		if (this.getCy() >= this.v_limitUpper + this.getHeight()/2) this.cy -= this.speed * delta;
	}
	...
	/**
		...
		O centro da coordenada do retângulo não deve ser menor que
		o limite da borda (v_limitUpper) menos a metade da sua altura
		(considerando que a coordenada Y esteja no centro do retângulo)
	...
	*/
	public void moveDown(long delta){
		if (this.getCy() <= this.v_limitLower - this.getHeight()/2) this.cy += this.speed * delta;
	}
	...
	*/
	...
	/**
	 * Método que devolve o limite superior da tela. Usada para que os jogadores não
	 * ultrapassem a borda superior.
	 * @return
	 */
	public double getV_limitUpper() {
		return this.v_limitUpper;
	}

	/**
	 * Método que devolve o limite inferior da tela. Usada para que os jogadores não
	 * ultrapassem a borda inferior.
	 * @return
	 */
	public double getV_limitLower() {
		return this.v_limitLower;
	}
}
